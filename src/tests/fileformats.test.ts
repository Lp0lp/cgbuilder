...
function makeCollection(
  beads: unknown[],
  atomNameMap: Record<number, string> = {},
): BeadCollection {
  return {
    beads,
    get assignedBeads() {
      return (beads as { atoms: unknown[] }[]).filter(
        (b) => b.atoms.length > 0,
      );
    },
    atomName(atom: { index: number }) {
      return atomNameMap[atom.index] ?? `A${atom.index}`;
    },
    expandedAtomNames(bead: {
      atoms: { index: number }[];
      expandedAtoms?: () => { index: number }[];
    }) {
