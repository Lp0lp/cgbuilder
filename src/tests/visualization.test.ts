...
vi.mock('../ngl.js', () => ({ NGL: {} }));
vi.mock('../rdkit.js', () => ({
  loadRDKit: () => Promise.reject(new Error('RDKit not needed in this test')),
}));
...
describe('typeColor', () => {
  ...
  it('falls back to light grey for an unknown or placeholder type', () => {
    ...
    expect(typeColor(undefined as unknown as string)).toEqual([
      0.75, 0.75, 0.75,
    ]);
  });
});
