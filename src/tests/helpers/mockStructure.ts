...
export function buildStructure(
  ...
  for (const bd of bondDefs) {
    const bond: BondProxy = {
      atomIndex1: bd.a,
      atomIndex2: bd.b,
      bondOrder: bd.order ?? 1,
    };
  ...
  const proxies: AtomProxy[] = atoms.map((atom) => ({
    ...
    positionToVector3: (): Vec3 =>
      ({ x: atom.x, y: atom.y, z: atom.z }) as Vec3,
    eachBond: (cb: (bond: BondProxy) => void) => {
      for (const b of bondsByAtom.get(atom.index)!) cb(b);
    },
  }));

  structure.eachAtom = (cb: (atom: AtomProxy) => void) => {
    for (const p of proxies) cb(p);
  };
...
export function regularPolygon(
  ...
  element: string,
  {
    ...
    cy = 0,
    startAngleDeg = 0,
  }: { cx?: number; cy?: number; startAngleDeg?: number } = {},
): AtomDef[] {
