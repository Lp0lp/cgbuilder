 ...
 */
function _maxMatching(nodes: number[], edgeList: [number, number][]): [number, number][] {
    const adj = new Map<number, number[]>(nodes.map((n): [number, number[]] => [n, []]));
    ...
    function backtrack(remaining: Set<number>, current: [number, number][]): void {
 ...
 */
function _connectedComponents(nodes: number[], adj: Map<number, number[]>): Set<number>[] {
 ...
 */
export function perceiveChemistry(structure: Structure | null | undefined): Chemistry {
    ...
    const link = (u: number, v: number): void => { if (!adj.has(u)) adj.set(u, []); adj.get(u)!.push(v); };
    structure.eachAtom((atom) => {
    ...
    for (const comp of _connectedComponents([...element.keys()], adj)) {
        const compElements = new Map<number, string>([...element].filter(([idx]) => comp.has(idx)));
    ...
    const degree = new Map<number, number>([...heavyAdj].map(([k, v]): [number, number] => [k, v.length]));
    ...
    const isHetero = (idx: number): boolean => ['N', 'O', 'S', 'SE'].includes(element.get(idx)!);
...
export function fragmentToSmiles(
    ...
    const edges = new Map<string, { a: number; b: number; order: number; aromatic: boolean }>();
 ...
 */
export function moleculeToSmiles(structure: Structure | null | undefined, chemistry: Chemistry): string | null {
    ...
    const adj = new Map<number, number[]>(heavyIndices.map((idx): [number, number[]] => [idx, []]));
    for (const atom of heavy) {
        if (typeof atom.eachBond !== 'function') continue;
        atom.eachBond((bond) => {
 ...
 * @param structure - NGL-style structure (eachAtom)
 */
export function weightedHeavyAtomCount(structure: Structure | null | undefined): number {
    let total = 0;
    if (structure && typeof structure.eachAtom === 'function') {
        structure.eachAtom((atom) => {
            const el = (atom.element || '').toUpperCase();
