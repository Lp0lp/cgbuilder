...
export function typeColor(beadType: string): [number, number, number] {
  ...
  switch (cls) {
    case 'C':
      return [0.55, 0.55, 0.55]; // grey   – apolar
    case 'N':
      return [0.29, 0.56, 0.85]; // blue   – intermediate
    case 'P':
      return [0.91, 0.3, 0.24]; // red    – polar
    case 'Q':
      return [0.95, 0.61, 0.07]; // amber  – charged
    case 'D':
      return [0.95, 0.61, 0.07]; // amber  – divalent (charged)
    case 'X':
      return [0.18, 0.8, 0.44]; // green  – halogen
    default:
      return [0.75, 0.75, 0.75]; // light grey – unknown/placeholder
  }
}
...
const OUTPUT_TABS = [
  {
    ...
    dlId: 'dl-ndx',
    copyId: 'copy-ndx',
    filename: 'cgbuilder.ndx',
    generate: generateNDX,
  },
  {
    outputId: 'map-output',
    dlId: 'dl-map',
    copyId: 'copy-map',
    filename: 'cgbuilder.map',
  ...
  },
  {
    ...
    dlId: 'dl-gro',
    copyId: 'copy-gro',
    filename: 'cgbuilder.gro',
    generate: generateGRO,
  },
  {
    outputId: 'bartender-output',
    dlId: 'dl-bartender',
    copyId: 'copy-bartender',
    filename: 'cgbuilder.bartender',
    generate: generateBartender,
  },
  {
    outputId: 'pycgtool-output',
    dlId: 'dl-pycgtool',
    copyId: 'copy-pycgtool',
    filename: 'cgbuilder_pycgtool.map',
    generate: generatePyCGTOOL,
  },
] as const;
...
export class Visualization {
  ...
  constructor(collection: BeadCollection, stage: Stage) {
    ...
    toggleCGLabels.onchange = (e) => {
      this.showCGLabels = (e.target as HTMLInputElement).checked;
      this.drawCG();
    };
    ...
    for (const tab of OUTPUT_TABS) {
      ...
      byId<HTMLButtonElement>(dlId).onclick = () =>
        download(filename, generate(this.collection));
      wireCopyButton(copyId, outputId);
    }
    byId<HTMLButtonElement>('dl-py').onclick = () =>
      download(
        'cgbuilder_assignments.py',
        generatePythonAssignments(this.collection),
      );
  ...
  attachRepresentation(component: StructureComponent): void {
    ...
    if (residueWarning)
      residueWarning.hidden = countResidues(component.structure) <= 1;

    const smilesEl = byId('smiles-output');
    if (smilesEl) {
      if (!chem.available) {
        smilesEl.textContent =
          'Needs a structure with explicit hydrogen atoms ' +
          '(e.g. a GROMACS .gro file, or a PDB run through a protonation tool).';
      } else {
   ...
   */
  async _loadPredictionDeps(): Promise<{
    RDKit: RDKitModule;
    canonTable: Record<string, number>;
  }> {
  ...
  _predictOneBead(
    ...
    if (!mol) {
      console.warn(
        `Bead "${bead.name}": invalid SMILES "${smiles}" — skipping`,
      );
      return;
    }
    ...
    const charge = bead.atoms.reduce(
      (s, a) => s + (chemistry.charges.get(a.index) ?? 0),
      0,
    );
    ...
    const heavyAtoms = bead.atoms.filter(
    ...
    const weightedHeavyCount = heavyAtoms.reduce(
      (s, a) => s + heavyAtomWeight(a.element),
      0,
    );
    const ringOrBranched = bead.atoms.some(
      (a) =>
    ...
    if (inRing) {
      ...
      for (const a of bead.atoms) {
        ...
        const k = fragmentToSmiles(bead.atoms, chemistry, {
          aromaticNotation: true,
          startIndex: a.index,
        });
        if (k) keys.add(k);
      }
      for (const k of keys) {
        if (canonTable[k] !== undefined) {
          lookupKey = k;
          tableVal = canonTable[k];
          break;
        }
      }
    ...
    bead.suggestedCharge =
      charge !== 0 && charge !== (bead.charge || 0) ? charge : null;
  }
  ...
  onBeadSelected(event: Event): void {
    ...
    const realTarget = findParentWithClass(
  ...
  onNameChange(event: Event): void {
    ...
    const index = this._beadIndexForNode(realTarget);
    if (index >= 0)
      this.collection.beads[index].name = (
  ...
  updateMappingStats(): void {
    ...
    const reset = (): void => {
      [heavyEl, beadsEl, mismatchEl].forEach((el) => {
        el.textContent = '—';
        el.className = '';
      });
    };
    ...
    const allTyped =
      nBeads > 0 && beads.every((b) => b.type && b.type !== 'TYPe');
  ...
  createBeadListItem(bead: Bead, isDuplicate = false): void {
    ...
    typeNode.oninput = (event) => {
      bead.type = (event.target as HTMLInputElement).value;
      this.updateName();
    };
    ...
    chargeNode.oninput = (event) => {
      bead.charge = (event.target as HTMLInputElement).value;
    ...
    predictNode.onclick = (e) => {
      e.stopPropagation();
      this.onPredictBeadType(bead, predictNode);
    };
    ...
    if (bead.atoms.length > 0) {
      ...
      for (let i = 0; i < bead.atoms.length; i++) {
        ...
        const w =
          bead.atomWeights && bead.atomWeights[atom.index]
            ? bead.atomWeights[atom.index]
            : 1;
        const subitem = document.createElement('li');
        subitem.appendChild(
          document.createTextNode(w > 1 ? `${name} ×${w}` : name),
        );
  ...
  createBeadList(): void {
    ...
    for (const bead of this.collection.beads)
      this.createBeadListItem(bead, dupes.has(bead.name ?? ''));
  }
  ...
  checkDuplicateNames(): void {
    ...
    for (const item of items) {
      ...
      if (!bead) break;
      const nameInput = (item as Element).querySelector<HTMLElement>(
        '.bead-name',
      );
      if (nameInput)
        nameInput.classList.toggle(
          'input-error',
          (counts.get(bead.name ?? '') || 0) > 1,
        );
    }
  }
  ...
  loadShakerMapping(text: string): void {
    ...
    if (!beadDefs.length) {
      console.warn('No beads found in mapping file');
      return;
    }
    ...
    for (const def of beadDefs) {
      ...
      for (const atomName of def.atoms) {
        ...
        if (idx !== undefined) {
          ...
          console.warn(
            `Mapping import: atom "${atomName}" not found in structure`,
          );
        }
      }
    }
  ...
  drawCG(rebuildSurface = true): void {
    ...
    for (const bead of this.collection.beads) {
      const color =
        bead === this.currentBead ? selectedColor : typeColor(bead.type);
      if (bead.atoms.length > 0) {
        const center = bead.center;
        shape.addSphere(center, color, 1.12, bead.name ?? '');
        if (this.showCGLabels)
          shape.addText(
            [center.x, center.y + 1.8, center.z],
            color,
            2.5,
            bead.name ?? '',
          );
